...
# Parallel settings
MAX_WORKERS = 2  # 测试：减少到2个worker以避免资源冲突（原为4）
...
def execute_test_step(step, session_id, timeout=30000):
    """执行单个测试步骤（使用独立session避免并行冲突）"""
    ...
    # 为所有agent-browser命令添加--session标志
    session_flag = f'--session {session_id}'

    try:
        if action == 'open':
            url = step.get('url', '')
            cmd = f'agent-browser {session_flag} open {url}'
            ...
            if 'load' in condition:
                cmd = f'agent-browser {session_flag} wait --load networkidle'
            elif 'selector' in condition:
                cmd = f"agent-browser {session_flag} wait {condition['selector']}"
            else:
                cmd = f'agent-browser {session_flag} wait {step.get("timeout", 5000) // 1000}'
            ...
            cmd = f'agent-browser {session_flag} snapshot -i'
            result = subprocess.run(
                cmd,
                shell=True,
                capture_output=True,
            ...
            for check in checks:
                ...
                if check_type == 'element_exists':
                    ...
                    # 使用--stdin避免shell引用问题
                    cmd = f'agent-browser {session_flag} eval --stdin <<< \'{js_code}\''
                    result = subprocess.run(
                        cmd,
                        shell=True,
                        capture_output=True,
            ...
            if screenshot_path:
                ...
                cmd = f'agent-browser {session_flag} screenshot {screenshot_path}'
                result = subprocess.run(
                    cmd,
                    shell=True,
                    capture_output=True,
    ...
    try:
        ...
        try:
            ...
            for step in test_config.get('steps', []):
                step_result, screenshot = execute_test_step(step, session_id, test_config.get('timeout', 30000))
            ...
            try:
                subprocess.run(
                    f'agent-browser --session {session_id} close',
                    shell=True,
                    capture_output=True,
                    timeout=5
                )
