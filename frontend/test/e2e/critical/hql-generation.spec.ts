...
test.describe('HQL预览模态框 - E2E测试', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.goto('/#/event-node-builder?game_gid=10000147', { timeout: 60000, waitUntil: 'commit' });
    await page.waitForTimeout(3000);

    // 添加字段到画布（HQL预览需要有字段才会显示按钮）
    await page.waitForSelector('.event-node-builder', { timeout: 60000 }).catch(() => {
      // Fallback to data-testid if class selector fails
      return page.waitForSelector('[data-testid="event-node-builder-workspace"]', { timeout: 60000 });
    });
