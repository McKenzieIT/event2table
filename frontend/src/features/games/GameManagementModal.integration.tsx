...
// ===== 示例 1: 在导航栏中集成 =====
const NavigationWithGameManager: React.FC = () => {
...
// ===== 示例 2: 在Dashboard页面集成 =====
const DashboardWithGameManager: React.FC = () => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
...
// ===== 示例 4: 与现有游戏列表集成 =====
const GamesListWithManager: React.FC = () => {
...
// ===== 示例 5: 使用快捷键触发 =====
const AppWithKeyboardShortcuts: React.FC = () => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
...
// ===== 示例 6: 带状态同步的集成 =====
const AppWithGameSync: React.FC = () => {
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [currentGame, setCurrentGame] = useState<Game | null>(null);
