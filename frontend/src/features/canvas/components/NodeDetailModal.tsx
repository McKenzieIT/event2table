...
export default function NodeDetailModal({
  ...
  node,
  onClose,
}: NodeDetailModalProps): React.JSX.Element | null {
        ...
        <div className="modal-body">
          {isEventNode && data.eventConfig ? (
            <>
              ...
              <div className="detail-section">
                ...
                <div className="detail-row">
                  ...
                  <span className="detail-value">
                    {(data.eventConfig as any)?.event_name_cn}
                  </span>
                </div>
                <div className="detail-row">
                  <span className="detail-label">事件标识:</span>
                  <span className="detail-value code">
                    {(data.eventConfig as any)?.event_name}
                  </span>
                </div>
                {(data.eventConfig as any)?.description && (
                  <div className="detail-row">
                  ...
                  {(data.baseFields || []).slice(0, 5).map((field, idx) => (
                    <div key={idx} className="field-item">
                      <span className="field-name">
                        {(field as any).alias || (field as any).field_name}
                      </span>
                      <span className="field-type">
                        {(field as any).field_type === "param" ? "参数" : "基础"}
                      </span>
                    </div>
                  ))}
              ...
              <div className="detail-section">
                ...
                <div className="detail-row">
                  ...
                  <span className="detail-value">{(data as any).nameCn || "-"}</span>
                </div>
              </div>
            </>
          ) : (
            <div className="detail-section">
              <h3>节点信息</h3>
              <p>{data.label}</p>
              {(data as any).description && <p>{(data as any).description}</p>}
            </div>
          )}
        </div>
        ...
        <div className="modal-footer">
          ...
          {isEventNode && data.configId && (
            <button
              ...
              onClick={() => {
                window.open(
                  `/event_node_builder?game_gid=${(window as any).gameData?.gid}&config_id=${data.configId}`,
                  "_blank",
                );
                onClose();
              }}
