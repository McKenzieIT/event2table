...
const ParameterTypeEditor: React.FC<ParameterTypeEditorProps> = ({
  isOpen,
  onClose,
  parameter,
  onSuccess
  ...
  const [newType, setNewType] = useState<'base' | 'param' | 'custom'>(parameter?.type || 'base');
  ...
  // Type options
  const typeOptions = [
    { value: 'base' as const, label: '基础字段 (base)' },
    { value: 'param' as const, label: '事件参数 (param)' },
