...
interface ParameterFiltersProps {
  gameGid?: number;
  mode: 'all' | 'common' | 'non-common';
  selectedEvent: string | null;
  onModeChange: (mode: 'all' | 'common' | 'non-common') => void;
  onEventChange: (event: string | null) => void;
  onViewCommonParams: () => void;
}

const ParameterFilters: React.FC<ParameterFiltersProps> = ({
  gameGid,
  mode,
  selectedEvent,
  onModeChange,
  ...
  const modeOptions = [
    { value: 'all' as const, label: '全部参数' },
    { value: 'common' as const, label: '公共参数' },
    { value: 'non-common' as const, label: '非公共参数' },
  ];
