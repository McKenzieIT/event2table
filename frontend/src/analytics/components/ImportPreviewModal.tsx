...
interface ImportPreviewModalProps {
  ...
  onConfirm: (selectedMatches: Set<number>) => void;
  onCancel: () => void;
}

export function ImportPreviewModal({ parameters, onConfirm, onCancel }: ImportPreviewModalProps): React.JSX.Element {
  const [selectedMatches, setSelectedMatches] = useState<Set<number>>(new Set());

  const matched = parameters.filter(p => p.libraryParam);
  const unmatched = parameters.filter(p => !p.libraryParam);

  const handleToggleMatch = (index: number): void => {
