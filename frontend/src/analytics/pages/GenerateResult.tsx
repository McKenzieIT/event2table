...
/**
 * TypeScript interfaces for Generate Result
 */
interface HqlResultResponse {
  success: boolean;
  data?: {
    hql_content?: string;
  };
...
function GenerateResult() {
  ...
  const [hqlResult, setHqlResult] = useState<string>("");
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [eventName, setEventName] = useState<string>("");
  const { success, error } = useToast();

  // 使用 useCallback 优化事件处理
  const generateTemplateHql = useCallback((event: string): void => {
    // 基于事件名生成模板HQL
  ...
  useEffect(() => {
    const fetchHqlResult = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const result: HqlResultResponse = await response.json();
