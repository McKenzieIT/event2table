...
| 文件路径 | 行数 | 问题描述 | 优先级 |
|---------|------|---------|-------|
| `Form/Form.test.tsx` | 1095 | 测试函数过多，需要优化代码组织 | 🔴 P0 |
| `Table/Table.tsx` | 854 | 组件逻辑复杂，需要拆分组件逻辑 | 🔴 P0 |
| `Modal/Modal.test.tsx` | 692 | 测试函数过多，需要优化代码组织 | 🟡 P1 |
| `Select/Select.tsx` | 590 | 组件逻辑较复杂 | 🟡 P1 |
| `FormTable.integration.test.tsx` | 843 | 集成测试函数过多，需要优化代码组织 | 🟡 P1 |

**说明**:
- **组件文件**：通过拆分逻辑、提取子组件、优化代码结构来管理
- **测试文件**：遵循"1个组件1个测试文件"原则，通过优化测试函数组织、提取辅助函数来管理
- **核心理念**：避免过度拆分文件，保持文件数量可控，重点关注代码内部组织质量
...
- ✅ 所有组件文件行数 < 500 行（通过优化逻辑组织实现）
- ✅ 所有测试文件行数 < 1200 行（遵循"1个组件1个测试文件"原则）
- ✅ 所有组件使用 TypeScript 严格模式
- ✅ 所有组件完成性能优化（React.memo、useCallback、useMemo）
- ✅ 所有组件支持可访问性（ARIA 属性）
- ✅ 所有测试文件有良好的代码组织（清晰的测试分组、辅助函数）
...
1. **预删除确认清单**
   ```bash
   # 步骤 1: 确认待删除目录的所有引用
   grep -r "import.*BaseModal" frontend/src --include="*.tsx" --include="*.ts" > /tmp/references.txt
   
   # 步骤 2: 人工审查引用列表
   echo "待删除组件的所有引用位置："
   cat /tmp/references.txt
   
   # 步骤 3: 确认引用已全部更新
   echo "请确认以上所有引用已更新到新组件，输入 YES 继续删除："
   read CONFIRMATION
...
2. **删除前备份**
   ```bash
   # 创建备份分支
   git checkout -b backup/before-delete-$(date +%Y%m%d-%H%M%S)
   git add .
   git commit -m "backup: 删除前的完整备份"
   git checkout main
   ```
...
2. **优化测试文件代码组织**
   - [ ] 优化 `Form.test.tsx` (1095行 → 保持1个文件，优化内部组织)
     - 提取测试辅助函数到 `test-utils/formTestUtils.ts`
     - 使用 `describe` 块清晰分组测试用例
     - 减少重复代码，提取公共测试逻辑
     - 控制单个测试函数行数 < 30 行
   - [ ] 优化 `Modal.test.tsx` (692行 → 保持1个文件，优化内部组织)
     - 提取测试辅助函数到 `test-utils/modalTestUtils.ts`
     - 使用 `describe` 块清晰分组测试用例
     - 减少重复代码，提取公共测试逻辑
   - [ ] 优化 `FormTable.integration.test.tsx` (843行 → 保持1个文件，优化内部组织)
     - 提取测试辅助函数和测试数据
     - 使用清晰的测试分组
     - 减少重复代码
...
**验收标准**:
- ✅ 测试文件保持"1个组件1个测试文件"原则
- ✅ 测试文件有良好的代码组织（清晰的测试分组、辅助函数）
- ✅ 单个测试函数行数 < 30 行
- ✅ 测试覆盖与优化前一致
- ✅ 所有测试通过
...
- ✅ **测试完善**: 测试覆盖率 ≥ 80%，所有核心组件有完整测试，遵循"1个组件1个测试文件"原则
...
- ✅ 所有组件文件 < 500 行
- ✅ 所有测试文件 < 1200 行（遵循"1个组件1个测试文件"原则）
- ✅ 所有组件遵循统一目录结构
- ✅ 所有测试文件有良好的代码组织（清晰的测试分组、辅助函数）
- ✅ 所有测试通过（使用分组测试方案）
- ✅ 无功能回归
...
在所有优化内容完成后，需要使用 **3 个 subagent 并行** 对前端组件库进行全面扫描，确保没有遗漏：

**扫描任务分配**（并行执行）:
```bash
# Subagent 1: 扫描组件目录结构和文件大小
任务：扫描所有组件目录，检查：
- 是否存在 > 500 行的组件文件
- 是否存在 > 1200 行的测试文件
- 是否存在未遵循统一目录结构的组件
...
# Subagent 2: 扫描性能优化情况
任务：扫描所有组件文件，检查：
- 是否所有组件都使用了 React.memo
- 是否所有事件处理器都使用了 useCallback
- 是否所有计算属性都使用了 useMemo
- 是否存在性能优化的遗漏

# Subagent 3: 扫描测试覆盖率和质量
任务：扫描所有组件测试，检查：
- 是否所有组件都有对应的测试文件
- 是否测试文件符合"1个组件1个测试文件"原则
- 是否存在测试覆盖率不足的组件
- 是否存在测试代码组织混乱的问题
```
...
- ✅ 3 个 subagent 并行扫描完成
- ✅ 生成完整的扫描报告（包含所有发现的问题）
- ✅ 所有发现的问题已记录到 Issue 跟踪系统
- ✅ 所有发现的问题已分配处理优先级

#### 📝 实施完成文档

在所有优化工作完成后，需要创建实施完成文档：
...
**文档提交要求**:
- ✅ 文档路径：`docs/implementation-reports/YYYY-MM-DD-component-library-optimization-completed.md`
- ✅ 文档已提交到 Git
- ✅ 文档已推送到 GitHub 远程仓库
- ✅ 文档已关联到相关 Issue 或 PR
...
- ✅ **强制要求**: 每一轮实施必须同时启动 3 个 subagent
- ✅ **任务分配**: 将当前轮次的任务拆分为 3 个独立子任务
- ✅ **并行执行**: 3 个 subagent 必须同时启动，不得串行执行
- ❌ **禁止行为**: 不得因为 token 限制或时间限制而减少 subagent 数量
- ❌ **禁止行为**: 不得因为 token 限制或时间限制而改为串行执行

**例外情况**:
- 仅当方案本身存在错误时，可以调整并行策略
- 调整前必须向用户说明原因并获得确认
- 调整后必须记录到实施日志中
...
- ✅ **强制要求**: 所有 `npm test` 命令必须使用分组参数
- ✅ **推荐参数**: `--testPathPattern="xxx" --maxWorkers=1`
- ❌ **禁止行为**: 不得直接运行 `npm test`（会触发 60s 超时）

#### 3. 实施完成文档约束

**完成后必须创建实施完成文档并推送到 GitHub**

- ✅ **强制要求**: 完成后必须创建 `docs/implementation-reports/YYYY-MM-DD-component-library-optimization-completed.md`
- ✅ **文档内容**: 必须包含完整的实施报告（见验收标准）
- ✅ **Git 提交**: 必须提交到 Git 并推送到 GitHub 远程仓库
- ❌ **禁止行为**: 不得遗漏文档创建或推送步骤
...
6. 进入下一轮次（重复步骤 1-5）
   ↓
7. 所有轮次完成后，启动 3 个 subagent 并行扫描查漏补缺
   ↓
8. 创建实施完成文档
   ↓
