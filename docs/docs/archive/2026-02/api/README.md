...
**🆕 重要更新** (2026-02-20):
- 所有API已完全切换到使用 `game_gid` 参数（不再使用 `game_id`）
- 新增 Service 层用于业务逻辑处理
- 新增错误处理中间件，统一错误响应格式
- 详细说明请参考: [后端优化报告](../optimization/FINAL_OPTIMIZATION_REPORT.md)
...
### game_gid vs game_id (2026-02-20完全切换)

**关键变更**: 所有API现在使用 `game_gid` 而非 `game_id`

| 参数类型 | 用途 | 示例 |
|---------|------|------|
| `game_gid` | 游戏业务GID（稳定的业务标识符） | `10000147` |
| `game_id` | 数据库自增ID（已废弃，仅用于内部主键） | `1` (不再使用) |

**影响范围**:
- ✅ 所有游戏相关API：`/api/games`, `/api/events`, `/api/parameters`
- ✅ 所有查询参数：`?game_gid=10000147`
- ✅ 所有请求体：`{"game_gid": 10000147, ...}`
- ✅ 数据库JOIN条件：`ON le.game_gid = g.gid`
...
**详细迁移指南**: [GAME_GID_MIGRATION_GUIDE.md](../development/GAME_GID_MIGRATION_GUIDE.md)
