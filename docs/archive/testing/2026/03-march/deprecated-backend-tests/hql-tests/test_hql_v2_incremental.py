...
def incremental_test_data(test_database):
    ...
    return {'game_gid': test_gid, 'event_id': test_event_id, 'event_name': 'test_login'}
...
class TestIncrementalHQLGenerator:
    ...
    def test_first_generation_no_previous_hql(self):
        ...
        fields = [Field(name='role_id', type=FieldType.BASE)]
        conditions = []

        generator = IncrementalHQLGenerator()
        result = generator.generate_incremental(
            events=events, fields=fields, conditions=conditions, previous_hql=None
        )
    ...
    def test_full_regeneration_on_event_change(self):
        ...
        result1 = generator.generate_incremental(
            events=events1, fields=fields1, conditions=[], previous_hql=None
        )

        # 更换事件
        events2 = [Event(name='event2', table_name='ods.table2')]
        result2 = generator.generate_incremental(
            events=events2, fields=fields1, conditions=[], previous_hql=result1['hql']
        )
    ...
    def test_incremental_on_field_modification(self):
        ...
        generator = IncrementalHQLGenerator()
        result1 = generator.generate_incremental(
            events=events, fields=fields1, conditions=conditions, previous_hql=None
        )

        # 修改字段(添加别名)
        fields2 = [Field(name='role_id', type=FieldType.BASE, alias='role')]
        result2 = generator.generate_incremental(
            events=events, fields=fields2, conditions=conditions, previous_hql=result1['hql']
        )
    ...
    def test_diff_detection_added_field(self):
        ...
        generator = IncrementalHQLGenerator()
        result1 = generator.generate_incremental(
            events=events, fields=fields1, conditions=[], previous_hql=None
        )
...
class TestIncrementalHQLAPI:
    ...
    def test_incremental_api_endpoint(self, client, incremental_test_data):
        ...
        request_data = {
            'events': [
                {
                    'game_gid': incremental_test_data['game_gid'],
                    'event_id': incremental_test_data['event_id'],
                }
            ],
        ...
        # 首次生成
        response1 = client.post('/hql-preview-v2/api/generate-incremental', json=request_data)

        assert (
            response1.status_code == 200
        ), f"Expected 200, got {response1.status_code}: {response1.get_json()}"
        ...
        # 第二次生成(应该检测到增量机会)
        request_data['fields'] = [{'fieldName': 'role_id', 'fieldType': 'base', 'alias': 'role'}]
        request_data['previous_hql'] = data1['data']['hql']

        response2 = client.post('/hql-preview-v2/api/generate-incremental', json=request_data)
