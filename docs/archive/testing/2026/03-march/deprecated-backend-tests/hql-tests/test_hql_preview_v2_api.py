...
class TestHQLPreviewV2API:
    ...
    def test_generate_endpoint_missing_events(self, client):
        """测试缺少events参数"""
        request_data = {'fields': [{'fieldName': 'role_id', 'fieldType': 'base'}]}
    ...
    def test_generate_endpoint_missing_fields(self, client):
        """测试缺少fields参数"""
        request_data = {'events': [{'game_gid': 10000147, 'event_id': 1}]}
...
class TestHQLPreviewV2APIValidate:
    ...
    def test_validate_valid_hql(self, client):
        """测试有效的HQL"""
        request_data = {'hql': 'SELECT role_id, account_id FROM table WHERE ds = \'${ds}\''}
    ...
    def test_validate_missing_select(self, client):
        """测试缺少SELECT的HQL"""
        request_data = {'hql': 'FROM table WHERE ds = \'${ds}\''}
    ...
    def test_validate_missing_partition_filter(self, client):
        """测试缺少分区过滤的HQL"""
        request_data = {'hql': 'SELECT role_id FROM table WHERE zone_id = 1'}
        ...
        # 要么有警告, 要么检查是否有相关提示
        has_partition_warning = any('分区' in str(warn.get('message', '')) for warn in warnings)
    ...
    def test_validate_unmatched_parentheses(self, client):
        ...
        # 验证包含括号相关的错误消息
        assert any(
            '括号' in err.get('message', '')
            or 'parenthesis' in err.get('message', '').lower()
            or 'parentheses' in err.get('message', '').lower()
            for err in errors
        )

    def test_validate_select_star_warning(self, client):
        """测试SELECT *警告"""
        request_data = {'hql': 'SELECT * FROM table WHERE ds = \'${ds}\''}
        ...
        if len(warnings) > 0:
            # 验证警告包含必要字段
            assert any(
                'line' in warn and 'column' in warn and 'message' in warn for warn in warnings
            )
    ...
    def test_validate_missing_from(self, client):
        """测试缺少FROM子句"""
        request_data = {'hql': 'SELECT role_id, account_id WHERE ds = \'${ds}\''}
...
class TestHQLPreviewV2APIRecommend:
    ...
    def test_recommend_fields_partial(self, client):
        """测试模糊匹配字段推荐"""
        response = client.get('/hql-preview-v2/api/recommend-fields?partial=zone')
