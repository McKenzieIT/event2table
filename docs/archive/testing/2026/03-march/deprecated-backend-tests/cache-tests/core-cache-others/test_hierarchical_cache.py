...
class TestCacheWarmer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 清空缓存统计 - 使用新CacheWarmer (2026-02-26)
        ...
        # 新系统使用stats字典, 直接重置即可
        warmer.stats = {"games_warmed": 0, "events_warmed": 0, "params_warmed": 0, "total_keys": 0}

    @patch("backend.services.cache.cache_warmup.fetch_all_as_dict")
    def test_warmup_games(self, mock_fetch):
        ...
        # 验证预热统计 - 使用新stats字段名
        stats = warmer.stats
        self.assertEqual(stats["games_warmed"], 2)
    ...
    def test_warmup_hot_events(self, mock_fetch):
        ...
        # 执行预热 - 使用新API
        warmer = CacheWarmer(cache=hierarchical_cache)
        warmer.warmup_recent_events(limit=100)

        # 验证预热统计 - 使用新stats字段名
        stats = warmer.stats
        self.assertEqual(stats["events_warmed"], 2)
    ...
    def test_periodic_warmup_thread(self, mock_fetch):
        ...
        # 启动定时预热(短间隔用于测试)- 使用新API
        warmer = CacheWarmer(cache=hierarchical_cache)
        warmer.start_periodic_warmup(interval_hours=0.001)  # ~3.6秒

        # 验证线程已启动 - 新系统使用相同属性名
        self.assertIsNotNone(warmer._warming_thread)
        self.assertTrue(warmer._warming_thread.is_alive())
        ...
        # 等待线程结束
        warmer._warming_thread.join(timeout=2)

        # 验证线程已停止
        self.assertFalse(warmer._warming_thread.is_alive())
