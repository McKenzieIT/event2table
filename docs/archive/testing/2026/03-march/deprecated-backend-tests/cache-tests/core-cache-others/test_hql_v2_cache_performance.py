...
class TestHQLV2CachePerformance:
    ...
    def test_cache_hit_on_second_request(self, client):
        ...
        if speedup < 0.5:
            ...
            warnings.warn(
                f"Cache hit was slower than cache miss ({speedup:.2f}x). "
    ...
    def test_cache_miss_on_different_config(self, client):
        ...
        # 第二个请求不应该命中缓存
        assert result2['data'].get('cached', False) == False, "Different config should miss cache"
    ...
    def test_performance_with_included_options(self, client):
        ...
        request_data = {
            ...
            'options': {'mode': 'single', 'include_comments': True, 'include_performance': True},
        }
