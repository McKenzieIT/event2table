 ...
 **功能描述**：提供游戏管理的可视化界面，支持游戏的创建、编辑、删除等操作。
 
 **核心能力**：
 - **创建游戏模态框**：表单界面创建新游戏，配置GID、名称、数据源
 - **编辑游戏模态框**：修改游戏配置信息
 - **删除确认模态框**：删除前的二次确认，显示关联数据影响
 - **批量操作模态框**：批量删除游戏的确认界面
 
 **技术实现**：
 - 前端组件：GameModal、GameForm、DeleteConfirmModal
 - 状态管理：Zustand管理模态框状态
 - 表单验证：React Hook Form + Zod验证
 - API集成：与游戏管理API端点集成
 
 #### 2.1.9 HQL预览V2 API ⭐
 
 **功能描述**：增强版HQL预览API，支持增量生成、调试模式和性能分析。
 
 **核心能力**：
 - **增量生成**：仅生成变更部分的HQL，提升生成效率
 - **调试模式**：输出详细的生成过程日志，便于问题排查
 - **性能分析**：记录HQL生成耗时、字段数量等性能指标
 - **版本对比**：对比不同版本的HQL差异
 
 **技术实现**：
 - API端点：`POST /api/hql/preview-v2`
 - 参数支持：`incremental`、`debug`、`performance_analysis`
 - 响应格式：包含HQL、性能指标、调试日志
 - 缓存策略：基于内容哈希的智能缓存
 
 #### 2.1.10 参数库管理 ⭐
 
 **功能描述**：管理可复用的参数定义模板，支持版本控制和验证规则。
 
 **核心能力**：
 - **参数模板**：定义标准参数模板（如role_id、account_id等通用参数）
 - **版本管理**：参数模板的版本控制，支持回滚
 - **验证规则**：定义参数值的验证规则（类型、格式、范围）
 - **模板导入**：从参数库快速导入参数到事件定义
 
 **技术实现**：
 - 数据模型：parameter_templates、parameter_versions、validation_rules
 - API端点：`GET /api/parameter-library`、`POST /api/parameter-library/templates`
 - 验证引擎：支持正则表达式、范围检查、枚举值验证
 - 版本控制：基于时间戳的版本管理
 
 #### 2.1.11 字段构建器配置管理 ⭐
 
 **功能描述**：管理字段构建器的配置项，支持配置的保存、加载和复用。
 
 **核心能力**：
 - **配置保存**：保存字段构建器的当前配置
 - **配置加载**：加载已保存的配置到构建器
 - **配置模板**：预定义常用字段配置模板
 - **配置导出**：导出配置为JSON格式，便于分享
 
 **技术实现**：
 - API端点：`GET /api/field-builder/configs`、`POST /api/field-builder/configs`
 - 数据存储：SQLite存储配置数据
 - 前端组件：FieldBuilderConfigPanel
 - 配置格式：JSON Schema定义配置结构
 
 #### 2.1.12 JOIN配置管理 ⭐
 
 **功能描述**：管理JOIN条件的配置，支持复杂JOIN关系的可视化配置。
 
 **核心能力**：
 - **JOIN条件配置**：配置JOIN类型（INNER/LEFT/RIGHT/FULL OUTER）
 - **ON条件编辑**：可视化编辑JOIN的ON条件
 - **多表JOIN**：支持多个表的级联JOIN
 - **JOIN预览**：实时预览JOIN后的数据结构
 
 **技术实现**：
 - 前端组件：JoinConfigEditor、JoinConditionBuilder
 - 数据模型：join_configs、join_conditions
 - API端点：`POST /api/join-configs`、`GET /api/join-configs/<id>`
 - 验证逻辑：检查JOIN条件的有效性和循环依赖
 
 #### 2.1.13 模板系统 ⭐
 
 **功能描述**：提供配置模板的管理功能，支持模板的创建、编辑、删除和应用。
 
 **核心能力**：
 - **模板创建**：从当前配置创建模板
 - **模板应用**：将模板应用到当前配置
 - **模板分类**：按类型分类模板（字段模板、JOIN模板、WHERE模板）
 - **模板分享**：导出/导入模板，便于团队协作
 
 **技术实现**：
 - 数据模型：templates、template_categories
 - API端点：`GET /api/templates`、`POST /api/templates`、`DELETE /api/templates/<id>`
 - 模板格式：JSON格式，包含配置数据和元数据
 - 权限控制：模板的创建者和可见性管理
 
 #### 2.1.14 缓存监控系统 ⭐
 
 **功能描述**：监控Redis缓存的状态和性能，提供缓存管理功能。
 
 **核心能力**：
 - **缓存状态监控**：实时查看缓存命中率、内存使用情况
 - **缓存键管理**：查看、删除特定的缓存键
 - **缓存统计**：按类型、游戏统计缓存使用情况
 - **缓存清理**：手动清理过期或无效缓存
 
 **技术实现**：
 - API端点：`GET /api/cache/stats`、`DELETE /api/cache/keys/<key>`
 - Redis集成：使用Redis作为缓存后端
 - 监控指标：命中率、内存使用、键数量
 - 前端组件：CacheMonitorDashboard
 
 #### 2.1.15 事件导入API增强 ⭐
 
 **功能描述**：增强事件导入功能，支持JSON格式导入和更灵活的数据映射。
 
 **核心能力**：
 - **JSON导入**：支持JSON格式的事件数据导入
 - **数据映射**：灵活的字段映射配置
 - **批量导入**：支持大批量事件数据导入
 - **导入验证**：导入前的数据验证和错误提示
 
 **技术实现**：
 - API端点：`POST /api/events/import`（支持Excel和JSON）
 - 数据验证：Pydantic Schema验证导入数据
 - 错误处理：详细的错误信息，支持部分导入成功
 - 性能优化：批量插入优化，支持事务回滚
...
- [US-IMPORT-001] 作为数据工程师，我希望从Excel批量导入事件定义，以便快速初始化系统
 ...
 #### 游戏管理模态框 (P1)
 - [US-MODAL-001] 作为数据工程师，我希望通过模态框创建新游戏，以便快速配置游戏项目
 - [US-MODAL-002] 作为数据工程师，我希望在删除游戏前看到关联数据影响，以便做出正确决策
 
 #### HQL预览V2 (P1)
 - [US-PREVIEW-001] 作为数据工程师，我希望使用增量生成模式，以便快速生成HQL
 - [US-PREVIEW-002] 作为数据工程师，我希望查看HQL生成的性能指标，以便优化配置
 
 #### 参数库管理 (P1)
 - [US-LIBRARY-001] 作为数据工程师，我希望创建参数模板，以便复用常用参数定义
 - [US-LIBRARY-002] 作为数据工程师，我希望管理参数模板版本，以便追踪变更历史
 
 #### 字段构建器配置 (P2)
 - [US-FIELD-CONFIG-001] 作为数据工程师，我希望保存字段构建器配置，以便快速恢复工作状态
 - [US-FIELD-CONFIG-002] 作为数据工程师，我希望导出配置为JSON，以便与团队分享
 
 #### JOIN配置管理 (P1)
 - [US-JOIN-001] 作为数据工程师，我希望可视化配置JOIN条件，以便构建复杂查询
 - [US-JOIN-002] 作为数据工程师，我希望预览JOIN结果，以便验证配置正确性
 
 #### 模板系统 (P2)
 - [US-TEMPLATE-001] 作为数据工程师，我希望创建配置模板，以便复用常见配置模式
 - [US-TEMPLATE-002] 作为数据工程师，我希望分享模板给团队成员，以便提高团队效率
 
 #### 缓存监控 (P2)
 - [US-CACHE-001] 作为系统管理员，我希望监控缓存状态，以便优化系统性能
 - [US-CACHE-002] 作为系统管理员，我希望手动清理缓存，以便解决缓存问题
 
 #### 事件导入增强 (P2)
 - [US-IMPORT-002] 作为数据工程师，我希望使用JSON格式导入事件，以便灵活导入数据
 - [US-IMPORT-003] 作为数据工程师，我希望在导入前验证数据，以便避免导入错误数据
 
 #### HQL模板引擎 (P1) ⭐ NEW
 - [US-TEMPLATE-ENGINE-001] 作为数据工程师，我希望使用模板变量替换生成动态HQL，以便支持不同分区条件
 - [US-TEMPLATE-ENGINE-002] 作为数据工程师，我希望自动提取模板变量，以便快速了解需要填充的参数
 - [US-TEMPLATE-ENGINE-003] 作为数据工程师，我希望验证模板变量完整性，以便避免生成错误的HQL
 
 #### 模板编辑器 (P1) ⭐ NEW
 - [US-TEMPLATE-EDITOR-001] 作为数据工程师，我希望通过可视化界面编辑模板，以便快速修改HQL模板
 - [US-TEMPLATE-EDITOR-002] 作为数据工程师，我希望模板编辑器支持变量高亮，以便识别动态部分
 - [US-TEMPLATE-EDITOR-003] 作为数据工程师，我希望实时预览模板渲染结果，以便验证模板正确性
 
 #### 异步任务系统 (P1) ⭐ NEW
 - [US-ASYNC-001] 作为数据工程师，我希望异步执行长时间HQL生成任务，以便不阻塞主界面
 - [US-ASYNC-002] 作为数据工程师，我希望查看异步任务的进度，以便了解执行状态
 - [US-ASYNC-003] 作为数据工程师，我希望取消正在执行的异步任务，以便节省系统资源
 
 #### SQL优化器建议 (P1) ⭐ NEW
 - [US-OPTIMIZER-001] 作为数据工程师，我希望查看SQL性能评分，以便了解HQL优化空间
 - [US-OPTIMIZER-002] 作为数据工程师，我希望获得优化建议，以便改进查询性能
 - [US-OPTIMIZER-003] 作为数据工程师，我希望一键应用优化建议，以便快速改进HQL
 
 #### 批量操作工具栏 (P2) ⭐ NEW
 - [US-BULK-001] 作为数据工程师，我希望批量选择事件进行操作，以便提高工作效率
 - [US-BULK-002] 作为数据工程师，我希望批量导出事件定义，以便备份和迁移
 - [US-BULK-003] 作为数据工程师，我希望批量验证事件配置，以便确保数据质量
 
#### Flow使用统计 (P2) ⭐ NEW
 - [US-FLOW-STATS-001] 作为数据工程师，我希望查看Flow模板的使用统计，以便了解哪些模板最常用
 - [US-FLOW-STATS-002] 作为数据工程师，我希望查看Flow中各节点的使用频率，以便优化模板设计
...
| 版本 | 日期 | 变更内容 | 负责人 |
|------|------|---------|-------|
| 1.4.0 | 2026-03-20 | **新增**: 阶段4功能补强（5个子项目，14项任务）<br>• 添加第2.1.16节：HQL模板引擎<br>• 添加第2.1.17节：模板编辑器<br>• 添加第2.1.18节：异步任务系统<br>• 添加第2.1.19节：SQL优化器建议<br>• 添加第2.1.20节：批量操作工具栏<br>• 添加第2.1.21节：Flow使用统计<br>• 新增18个用户故事（US-TEMPLATE-ENGINE/TEMPLATE-EDITOR/ASYNC/OPTIMIZER/BULK/FLOW-STATS）<br>• V2命名统一、HQL分区条件统一、前端性能优化 | Claude |
| 1.3.0 | 2026-02-20 | **优化**: V2 API统一和废弃组件清理<br>• HQL预览V2成为唯一生产版本<br>• 清理重复API文件和组件<br>• 更新功能优先级清单 | Claude |
