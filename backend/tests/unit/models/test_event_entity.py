...
class TestEventEntityValidation:
    ...
    def test_valid_event_creation_minimal(self):
        ...
        event = EventEntity(game_gid=90000001, event_name='login')
    ...
    def test_xss_protection_in_event_name(self):
        ...
        event = EventEntity(game_gid=90000001, event_name='<script>alert("xss")</script>login')
    ...
    def test_xss_protection_in_event_name_cn(self):
        ...
        event = EventEntity(
            game_gid=90000001, event_name='login', event_name_cn='<img src=x onerror=alert(1)>登录'
        )
        # HTML should be escaped
    ...
    def test_name_alias_for_event_name(self):
        ...
        # Using 'name' (alias)
        event1 = EventEntity(game_gid=90000001, name='login')  # Using alias
        assert event1.event_name == 'login'

        # Using 'event_name' (field name)
        event2 = EventEntity(game_gid=90000001, event_name='login')  # Using field name
        assert event2.event_name == 'login'

    def test_name_cn_alias_for_event_name_cn(self):
        """Test 'name_cn' alias works for event_name_cn"""
        # Using 'name_cn' (alias)
        event1 = EventEntity(game_gid=90000001, event_name='login', name_cn='登录')  # Using alias
        assert event1.event_name_cn == '登录'

        # Using 'event_name_cn' (field name)
        event2 = EventEntity(
            game_gid=90000001, event_name='login', event_name_cn='登录'  # Using field name
        )
    ...
    def test_game_gid_must_be_non_negative(self):
        ...
        with pytest.raises(ValidationError):
            EventEntity(game_gid=-1, event_name='login')  # Invalid

    def test_game_gid_accepts_string(self):
        """Test game_gid accepts string and converts to int"""
        event = EventEntity(game_gid="90000001", event_name='login')  # String
    ...
    def test_event_name_min_length(self):
        ...
        with pytest.raises(ValidationError):
            EventEntity(game_gid=90000001, event_name='')  # Too short

    def test_event_name_max_length(self):
        """Test event_name maximum length validation"""
        with pytest.raises(ValidationError):
            EventEntity(game_gid=90000001, event_name='x' * 101)  # Too long (max 100)

    def test_event_name_cn_max_length(self):
        """Test event_name_cn maximum length validation"""
        with pytest.raises(ValidationError):
            EventEntity(
                game_gid=90000001, event_name='login', event_name_cn='x' * 101  # Too long (max 100)
            )
...
class TestEventEntityProperties:
    ...
    def test_name_cn_property(self):
        ...
        event = EventEntity(game_gid=90000001, event_name='login', event_name_cn='登录')
...
class TestEventEntitySerialization:
    ...
    def test_model_dump_returns_dict(self):
        ...
        event = EventEntity(id=1, game_gid=90000001, event_name='login')
    ...
    def test_model_dump_json_roundtrip(self):
        ...
        original = EventEntity(id=1, game_gid=90000001, event_name='login', event_name_cn='登录')
    ...
    def test_from_dict(self):
        ...
        data = {'id': 1, 'game_gid': 90000001, 'event_name': 'login', 'event_name_cn': '登录'}
...
class TestEventEntityFieldValidation:
    ...
    def test_event_name_whitespace_trimmed(self):
        ...
        event = EventEntity(game_gid=90000001, event_name='  login  ')  # Whitespace
    ...
    def test_event_name_cn_whitespace_trimmed(self):
        ...
        event = EventEntity(
            game_gid=90000001, event_name='login', event_name_cn='  登录  '  # Whitespace
        )
    ...
    def test_id_can_be_none(self):
        ...
        event = EventEntity(id=None, game_gid=90000001, event_name='login')  # Explicit None
        assert event.id is None

    def test_table_name_not_persisted(self):
        """Test table_name is excluded from serialization"""
        event = EventEntity(
            id=1, game_gid=90000001, event_name='login', table_name='ieu_ods.ods_90000001_login'
        )
        data = event.model_dump()
        assert 'table_name' not in data
    ...
    def test_param_count_not_persisted(self):
        ...
        event = EventEntity(id=1, game_gid=90000001, event_name='login', param_count=5)
        data = event.model_dump()
        assert 'param_count' not in data
...
class TestEventEntityEdgeCases:
    ...
    def test_special_characters_in_event_name(self):
        ...
        for name in special_names:
            event = EventEntity(game_gid=90000001, event_name=name)
