...
⚠️ DEPRECATED: BatchImportManager的_prepare_*方法是占位实现

这些测试已跳过, 因为依赖的功能是占位实现. 
参考: backend/services/bulk_operations/batch_import_manager.py:43
"""
...
class TestCreateBatchIntegration:
    ...
    def test_prepare_event_record(self):
        """测试事件记录准备功能"""
        event_data = {'event_name': 'test_login', 'event_name_cn': '测试登录', 'parameters': []}
        ...
        try:
            record = batch_import_manager._prepare_event_record(event_data, game_gid, category_id)
