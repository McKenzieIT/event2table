...
class MigrationTestSuite:
    ...
    def test_database_schema(self):
        ...
        for table in tables:
            ...
            if has_column:
                ...
                self.log_result(
                    "Database Schema", f"{table} has game_gid column", False, "Column missing"
                )
                all_passed = False
    ...
    def test_database_data_integrity(self):
        ...
        for table in tables_to_check:
            ...
            if total == 0:
                ...
                self.log_result(
                    "Data Integrity", f"{table} data integrity", True, f"{total} rows, all valid"
                )
            else:
                self.log_result(
                    "Data Integrity",
                    f"{table} data integrity",
                    False,
                    f"{nulls}/{total} NULL game_gid",
                )
                all_passed = False
    ...
    def test_database_foreign_keys(self):
        ...
        if orphaned_events == 0:
            ...
            self.log_result(
                "Foreign Keys", "log_events → games", False, f"{orphaned_events} orphaned events"
            )
        ...
        if orphaned_nodes == 0:
            ...
            self.log_result(
                "Foreign Keys",
                "event_node_configs → games",
                False,
                f"{orphaned_nodes} orphaned nodes",
            )
    ...
    def test_util_functions(self):
        ...
        try:
            ...
            if has_events:
                ...
                self.log_result(
                    "Utils", "check_game_has_events(10000147)", False, "No events found"
                )
                all_passed = False
        except Exception as e:
        ...
        try:
            ...
            if games and len(games) > 0:
                self.log_result(
                    "Utils", "get_games_with_event_counts()", True, f"{len(games)} games"
                )
            else:
        ...
        try:
            ...
            if event and event.get('game_gid'):
                self.log_result(
                    "Utils", "get_event_with_game_info(1)", True, f"game_gid={event['game_gid']}"
                )
            else:
                self.log_result(
                    "Utils",
                    "get_event_with_game_info(1)",
                    False,
                    "Event not found or missing game_gid",
                )
                all_passed = False
        except Exception as e:
        ...
        try:
            ...
            if categories is not None:  # Can be empty list
                self.log_result(
                    "Utils",
                    "get_categories_by_game(10000147)",
                    True,
    ...
    def test_api_endpoints(self):
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    if len(games) > 0:
                        ...
                        self.log_result(
                            "API", "/api/games", True, f"{len(games)} games, {total_events} events"
                        )
                    else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_result(
                    "API", "/api/dashboard-statistics", False, f"HTTP {response.status_code}"
                )
                all_passed = False
        except Exception as e:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/event_node_builder/api/stats?game_gid=10000147", timeout=10
            )
            if response.status_code == 200:
                self.log_result("API", "/event_node_builder/api/stats", True, "HTTP 200")
            else:
                self.log_result(
                    "API", "/event_node_builder/api/stats", False, f"HTTP {response.status_code}"
                )
                all_passed = False
        except Exception as e:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/event_node_builder/api/search?game_gid=10000147", timeout=10
            )
            if response.status_code == 200:
                data = response.json()
                nodes = data.get('data', {}).get('nodes', [])
                self.log_result(
                    "API", "/event_node_builder/api/search", True, f"{len(nodes)} nodes"
                )
            else:
                self.log_result(
                    "API", "/event_node_builder/api/search", False, f"HTTP {response.status_code}"
                )
                all_passed = False
        except Exception as e:
        ...
        for path, name in pages:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_result(
                        "Frontend", f"{name} ({path})", False, f"HTTP {response.status_code}"
                    )
                    all_passed = False
            except Exception as e:
    ...
    def test_end_to_end_scenarios(self):
        ...
        try:
            ...
            response = requests.get(
                f"{self.base_url}/event_node_builder/api/stats?game_gid={game_gid}", timeout=10
            )
            if response.status_code != 200:
