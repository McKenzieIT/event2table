...
class TestGamesAPIList:
    ...
    def test_list_games_includes_statistics(self, client):
        ...
        if len(games) > 0:
            ...
            assert 'gid' in game
            assert 'name' in game
            assert 'ods_db' in game
            # 注意: 统计字段(event_count等)可能尚未实现, 暂不测试
...
class TestGamesAPICreate:
    ...
    def test_create_game_invalid_gid(self, client):
        ...
        response = client.post(
            '/api/games', json={'gid': -1, 'name': 'Test Game', 'ods_db': 'test_db'}
        )
    ...
    def test_create_game_empty_ods_db(self, client):
        """测试创建游戏时ODS数据库名称为空"""
        response = client.post(
            '/api/games', json={'gid': 999999, 'name': 'Test Game', 'ods_db': ''}
        )
