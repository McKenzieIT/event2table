...
try:
    ...
    print(
        f"[CONFTEST] backend.__path__: {backend.__path__ if hasattr(backend, '__path__') else 'N/A'}"
    )
    ...
    if not event:
        ...
        """,
            (
                ...
                "dwd.v_dwd_10000147_test_role_online_di",
            ),
        )
        db.commit()
        event = db.execute("SELECT * FROM log_events WHERE id = ?", (test_event_id,)).fetchone()
...
@pytest.fixture
def sample_game_no_events(db):
    """创建没有关联事件的测试游戏"""
    import random
    import time

    # 使用唯一GID避免冲突
    unique_gid = 90000000 + int(str(int(time.time() * 1000))[-4:]) + random.randint(0, 9999)
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?)
        """,
        (unique_gid, "Test Game No Events", "test_ods"),
    )
    game_id = cursor.lastrowid
    db.commit()
...
@pytest.fixture
def sample_game_with_events(db):
    """创建有关联事件的测试游戏"""
    import random
    import time
    ...
    cursor.execute(
        ...
        VALUES (?, ?, ?)
        """,
        (unique_gid, "Test Game With Events", "test_ods"),
    )
    game_id = cursor.lastrowid
    db.commit()
...
@pytest.fixture
def sample_events(db, sample_game_with_events):
    """为测试游戏创建多个测试事件"""
    events = []
    ...
    for i in range(3):
        ...
        cursor.execute(
            ...
            """,
            (
                sample_game_with_events["id"],
                sample_game_with_events["gid"],
                f"test_event_{i}",
                f"测试事件{i}",
                "test_ods",
                "dwd_test",
...
@pytest.fixture
def sample_node_configs(db, sample_game_with_events):
    """为测试游戏创建Canvas节点配置"""
    configs = []
    ...
    for i in range(2):
        ...
        cursor.execute(
            """
            INSERT INTO event_node_configs (game_gid, name_en, name_cn, event_id, base_fields, filter_conditions, description)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                sample_game_with_events["gid"],
                f"test_node_{i}",  # name_en
                f"测试节点{i}",  # name_cn
                1,  # 使用固定的event_id
                '{"fields": []}',  # base_fields
                '{"conditions": []}',  # filter_conditions
                f"Test node config {i}",  # description
            ),
        )
        ...
        config = db.execute(
            "SELECT * FROM event_node_configs WHERE id = ?", (config_id,)
        ).fetchone()
...
@pytest.fixture
def sample_event_with_params(db, sample_game_with_events):
    """创建带有参数的测试事件"""
    cursor = db.cursor()
    cursor.execute(
        """
    ...
    for i in range(3):
        cursor.execute(
            ...
            VALUES (?, ?, ?, ?, ?)
            """,
            (event_id, f"param_{i}", f"参数{i}", 1, 1),
        )
    db.commit()
