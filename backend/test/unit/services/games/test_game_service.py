...
def test_get_all_games_returns_entities(game_service):
    ...
    with patch.object(game_service.game_repo, 'find_all') as mock_find_all:
        mock_find_all.return_value = [
            GameEntity(id=1, gid=90000001, name="Test Game 1", ods_db="test_ods", dwd_prefix="dwd"),
            GameEntity(id=2, gid=90000002, name="Test Game 2", ods_db="test_ods", dwd_prefix="dwd"),
        ]
...
def test_get_game_by_gid_returns_entity(game_service):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find:
        mock_find.return_value = GameEntity(
            id=1, gid=90000001, name="Test Game", ods_db="test_ods", dwd_prefix="dwd"
        )
...
def test_create_game_success(game_service, test_game_entity):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service.game_repo, 'create'
    ) as mock_create, patch.object(game_service, 'invalidator') as mock_invalidator, patch.object(
        type(game_service), 'bloom_filter', new_callable=PropertyMock
    ) as mock_bloom:
        # Setup mocks
...
def test_create_game_invalid_ods_db(game_service):
    ...
    try:
        with pytest.raises(ValidationError, match="ods_db必须是以下值之一"):
            GameEntity(gid=90000001, name="Test Game", ods_db="invalid_db")  # Invalid ods_db
    finally:
        # Restore environment
...
def test_star001_protection_delete(game_service):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service.game_repo, 'delete'
    ) as mock_delete:
        # Mock game exists
        mock_find.return_value = GameEntity(
            id=1, gid=STAR001_GID, name="STAR001", ods_db="ieu_ods", dwd_prefix="dwd"
        )
...
def test_update_game_success(game_service, test_game_entity):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service.game_repo, 'update'
    ) as mock_update, patch.object(game_service, 'invalidator') as mock_invalidator, patch.object(
        game_service, 'get_game_by_gid'
        ...
        mock_get.return_value = GameEntity(
            id=1, gid=90000001, name="Updated Game", ods_db="test_ods", dwd_prefix="dwd"
        )
        mock_invalidator.invalidate_pattern = MagicMock()
        mock_bloom.return_value = MagicMock()
...
def test_delete_game_success(game_service):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service.game_repo, 'delete'
    ) as mock_delete, patch.object(game_service, 'invalidator') as mock_invalidator:
        # Setup mocks
        mock_find.return_value = GameEntity(
            id=1, gid=90000001, name="Test Game", ods_db="test_ods", dwd_prefix="dwd"
        )
...
def test_batch_delete_games_success(game_service):
    ...
    with patch.object(game_service.game_repo, 'batch_delete') as mock_batch_delete, patch.object(
        game_service, 'invalidator'
    ) as mock_invalidator:
        # Setup mocks
...
def test_batch_update_games_success(game_service):
    ...
    with patch.object(
        game_service.game_repo, 'batch_update_by_gid'
    ) as mock_batch_update, patch.object(game_service, 'invalidator') as mock_invalidator:
        # Setup mocks
...
def test_update_game_repository_error(game_service, test_game_entity):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service.game_repo, 'update'
    ) as mock_update, patch.object(game_service, 'get_game_by_gid') as mock_get, patch.object(
        type(game_service), 'bloom_filter', new_callable=PropertyMock
    ) as mock_bloom:
        # Setup mocks
        mock_find.return_value = test_game_entity
...
def test_cascade_delete_game_with_data_no_force(game_service):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service, 'check_deletion_impact'
    ) as mock_impact:
        # Setup mocks
        mock_find.return_value = GameEntity(
            id=1, gid=90000001, name="Test Game", ods_db="test_ods", dwd_prefix="dwd"
...
def test_cascade_delete_game_with_data_force(game_service):
    ...
    with patch.object(game_service.game_repo, 'find_by_gid') as mock_find, patch.object(
        game_service, 'check_deletion_impact'
    ) as mock_impact, patch('backend.core.database.database.get_db_connection') as mock_get_conn:
        # Setup mocks
        mock_find.return_value = GameEntity(
            id=1, gid=90000001, name="Test Game", ods_db="test_ods", dwd_prefix="dwd"
        )
