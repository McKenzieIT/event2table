...
class ParametersPerformanceTest:
    ...
    def setup_test_data(self):
        ...
        try:
            ...
            if len(templates) < 3:
                ...
                cursor.execute(
                    "INSERT INTO param_templates (template_name, base_type) VALUES (?, ?)",
                    ("string", "string"),
                )
            ...
            for event_id in event_ids:
                for param_name in param_names:
                    ...
                    """,
                        (event_id, param_name, f"参数_{param_name}", template_id),
                    )
                    batch_size += 1
    ...
    def print_report(self, query_results: Dict, cache_results: Dict, e2e_results: Dict):
        ...
        print(f"  Optimized Avg:    {query_results['avg']:.2f} ms")
        print(
            f"  Status:           {'✅ PASS' if query_results['avg'] < sla_threshold else '❌ FAIL'}"
        )

        print(f"\n  P95 Baseline:     {baseline_p95:.2f} ms")
        print(f"  P95 Optimized:    {query_results['p95']:.2f} ms")
        print(
            f"  Status:           {'✅ PASS' if query_results['p95'] < 150 else '⚠️  NEEDS IMPROVEMENT'}"
        )
