...
def test_link_to_library_success(client):
    ...
    param_id = fetch_one_as_dict(
        "SELECT id FROM event_params WHERE param_name = ?", (unique_param_name,)
    )['id']
    library_id = fetch_one_as_dict(
        "SELECT id FROM param_library WHERE param_name = ?", (unique_param_name,)
    )['id']

    # 调用 API
    response = client.post(
        f'/api/event-params/{param_id}/link-library',
