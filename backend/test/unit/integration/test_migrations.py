...
class TestDatabaseMigrations(unittest.TestCase):
    ...
    def test_migration_from_v0_to_v19(self):
        """Test full migration from version 0 to 19"""
    ...
    def test_migration_v1_category_id(self):
        ...
        conn = sqlite3.connect(str(self.db_path))
        self.assertTrue(
            self._column_exists(conn, "log_events", "category_id"),
            "category_id column should exist",
        )
    ...
    def test_migration_v2_event_category_relations(self):
        ...
        conn = sqlite3.connect(str(self.db_path))
        self.assertTrue(
            self._table_exists(conn, "event_category_relations"),
            "event_category_relations table should exist",
        )
        conn.close()
    ...
    def test_migration_v4_icon_path(self):
        ...
        conn = sqlite3.connect(str(self.db_path))
        self.assertTrue(
            self._column_exists(conn, "games", "icon_path"),
            "icon_path column should exist in games table",
        )
        conn.close()
    ...
    def test_migration_v5_edit_tracking(self):
        ...
        for column in ["is_user_edited", "edit_notes", "original_content"]:
            self.assertTrue(
                self._column_exists(conn, "hql_statements", column),
                f"{column} column should exist in hql_statements",
            )
        conn.close()
    ...
    def test_migration_v18_game_gid(self):
        ...
        # Insert test game
        cursor.execute(
            "INSERT INTO games (gid, name, ods_db) VALUES (10000147, 'Test Game', 'test_ods')"
        )
        ...
        cursor.execute(
            "INSERT INTO log_events (id, game_id, event_name) VALUES (?, ?, 'test_event')",
            (event_id, game_id),
        )
        ...
        conn = sqlite3.connect(str(self.db_path))
        self.assertTrue(
            self._column_exists(conn, "log_events", "game_gid"),
            "game_gid column should exist",
        )
