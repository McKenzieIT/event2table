...
class TestEventsAPI(APITestCase):
    ...
    def test_05_get_event_detail(self):
        ...
        game_gid = self.test_game_gid if self.test_game_gid else 10000147
        response = self.client.get(f"/api/events/{event_id}?game_gid={game_gid}")
        self.assert_response_success(response)
...
class TestParameterAliasesAPI(APITestCase):
    ...
    def test_03_update_parameter_alias(self):
        ...
        cursor.execute(
            "DELETE FROM parameter_aliases WHERE alias = ? AND param_id = ?",
            ("test_alias", 1),
        )
        cursor.execute(
            """
