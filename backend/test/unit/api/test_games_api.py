...
# Tests should use the Flask test client to hit the endpoints directly
...
class TestGamesAPIImport:
    ...
    def test_import(self):
        """Test module blueprint can be imported"""
        # The new API structure uses Flask blueprints
        assert api_bp is not None
        assert api_bp.name == "api"
        assert len(api_bp.deferred_functions) > 0  # Has routes registered
...
class TestCreateGame:
    ...
    def test_create_game_missing_fields(self, app, client):
        ...
        response = client.post(
            "/api/games", json={"name": "Test Game"}, content_type="application/json"
        )
