...
class TestGenericRepository:
    ...
    def test_find_where_with_limit(self):
        ...
        assert (
            len(result) <= 5
        ), f"find_where with limit=5 should return at most 5 records, got {len(result)}"
    ...
    def test_find_first_where_returns_single_record(self):
        ...
        assert result is None or isinstance(
            result, dict
        ), "find_first_where should return a dict or None"
    ...
    def test_delete_batch_deletes_multiple_records(self):
        ...
        assert (
            deleted_count == 0
        ), f"delete_batch with non-existent IDs should return 0, got {deleted_count}"
    ...
    def test_update_batch_updates_multiple_records(self):
        ...
        assert (
            updated_count == 0
        ), f"update_batch with non-existent IDs should return 0, got {updated_count}"
    ...
    def test_create_batch_creates_multiple_records(self):
        ...
        assert len(inserted_ids) == len(
            test_records
        ), f"Expected {len(test_records)} IDs, got {len(inserted_ids)}"
        assert all(
            isinstance(id, int) and id > 0 for id in inserted_ids
    ...
    def test_create_batch_with_partial_fields(self):
        ...
        test_records = [{"name": f"TEST_CAT_PARTIAL_{uuid.uuid4().hex[:8]}"}]
