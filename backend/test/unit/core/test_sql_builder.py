...
class TestAggregateFunctionBuilder:
    ...
    def test_custom_builder(self):
        ...
        sql = AggregateFunctionBuilder.build_aggregate_sql(
            'MYFUNC', 'field', 'alias', custom_builder=custom_builder
        )
...
class TestNormalizeFieldList:
    ...
    def test_normalize_simple_fields(self):
        ...
        fields = [{'fieldName': 'user_id', 'alias': 'uid', 'type': 'int'}]

        normalized = normalize_field_list(fields)

        assert len(normalized) == 1
        assert normalized[0] == {'name': 'user_id', 'alias': 'uid', 'type': 'int'}
    ...
    def test_normalize_with_name_key(self):
        ...
        fields = [{'name': 'field_a', 'alias': 'alias_a'}]
    ...
    def test_normalize_with_fieldtype(self):
        ...
        fields = [{'fieldName': 'x', 'fieldType': 'int'}]
...
class TestJoinBuilder:
    ...
    def test_build_inner_join(self):
        ...
        sql = JoinBuilder.build_join_clause('INNER', 'events', 'users', 'events.user_id = users.id')
    ...
    def test_build_left_join(self):
        ...
        sql = JoinBuilder.build_join_clause('LEFT', 'events', 'users', 'events.user_id = users.id')
    ...
    def test_build_right_join(self):
        ...
        sql = JoinBuilder.build_join_clause('RIGHT', 'events', 'users', 'events.user_id = users.id')
    ...
    def test_build_full_join(self):
        ...
        sql = JoinBuilder.build_join_clause('FULL', 'events', 'users', 'events.user_id = users.id')
    ...
    def test_build_cross_join(self):
        ...
        sql = JoinBuilder.build_join_clause('CROSS', 'events', 'users', 'events.user_id = users.id')
    ...
    def test_build_join_default_inner(self):
        ...
        sql = JoinBuilder.build_join_clause('UNKNOWN', 'A', 'B', 'A.id = B.id')
