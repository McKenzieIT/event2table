...
class TestParameterRepositoryCache:
    ...
    def test_create_param_invalidates_cache(self, repo, mock_param_data):
        """
        Test that create_param invalidates cache
        Expected: After creating a parameter, cache should be cleared
        """
        with patch('backend.models.repositories.parameters.fetch_all_as_dict') as mock_fetch, \
             patch('backend.models.repositories.parameters.execute_write') as mock_write:

            mock_fetch.return_value = [mock_param_data]
            mock_write.return_value = 1  # Return new param ID

            # First call (populates cache)
            repo.get_paginated_params(page=1, per_page=50)

            # Create new parameter (should invalidate cache)
            repo.create_param({
                'event_id': 100,
                'name': 'new_param',
                'field_type': 'int'
            ...
            # This call should hit database again (cache was invalidated)
    ...
    def test_update_param_invalidates_cache(self, repo, mock_param_data):
        """
        Test that update_param invalidates cache
        Expected: After updating a parameter, cache should be cleared
        """
        with patch('backend.models.repositories.parameters.fetch_all_as_dict') as mock_fetch, \
             patch('backend.models.repositories.parameters.execute_write') as mock_write:

            mock_fetch.return_value = [mock_param_data]
            mock_write.return_value = 1  # Return affected rows

            # First call (populates cache)
            repo.get_paginated_params(page=1, per_page=50)

            # Update parameter (should invalidate cache)
            repo.update_param(param_id=1, param_data={'name': 'updated_param'})
    ...
    def test_cache_performance_improvement(self, repo, mock_param_data):
        ...
        with patch('backend.models.repositories.parameters.fetch_all_as_dict') as mock_fetch:
            ...
            assert time_hit < time_miss / 10, \
                f"Cache hit ({time_hit:.3f}s) should be at least 10x faster than cache miss ({time_miss:.3f}s)"
