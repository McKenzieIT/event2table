...
class AlertingSystem:
    ...
    def check_metric(self, metric_type: AlertType, value: float, context: Dict | None = None):
        ...
        with self._lock:
            for rule_name, rule in self._alert_rules.items():
                ...
                is_exceeded = (comparison == 'above' and value > threshold) or (
    ...
    def _resolve_alert(self, rule_name: str):
        ...
        if alert and not alert.resolved:
            ...
            logger.info(f"ALERT RESOLVED: {alert.message}", extra={'alert_id': alert.id})
    ...
    def get_alert_history(self, hours: int = 24) -> List[Alert]:
        ...
        with self._lock:
            return [alert for alert in self._alerts.values() if alert.timestamp >= cutoff]
