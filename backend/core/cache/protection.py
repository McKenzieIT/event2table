...
class CacheProtection:
    ...
    def get_with_bloom_filter(
        self, pattern: str, func: Callable, ttl: Optional[int] = None, **kwargs
    ) -> Any:
        """
        使用布隆过滤器防止缓存穿透
        ...
        try:
            ...
            if data is not None:
                ...
                hierarchical_cache.set(
                    pattern, self._EMPTY_MARKER, ttl=CacheConfig.CACHE_EMPTY_TTL, **kwargs
                )
    ...
    def set_with_random_ttl(
        self, pattern: str, data: Any, base_ttl: int = 300, jitter_pct: float = 0.2, **kwargs
    ):
        """
        使用随机TTL防止缓存雪崩
