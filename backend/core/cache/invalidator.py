...
class CacheInvalidatorEnhanced:
    ...
    def invalidate_event_related(self, event_id: int, game_gid: int) -> Set[str]:
        ...
        try:
            ...
            logger.info(
                f"事件关联失效: event_id={event_id}, game_gid={game_gid}, {len(invalidated_keys)}个键"
            )
    ...
    def invalidate_parameter_related(self, param_id: int, event_id: int, game_gid: int) -> Set[str]:
        ...
        try:
            ...
            logger.info(
                f"参数关联失效: param_id={param_id}, event_id={event_id}, game_gid={game_gid}, {len(invalidated_keys)}个键"
            )
    ...
    def invalidate_category_related(
        self, category_id: int, game_gid: Optional[int] = None
    ) -> Set[str]:
        """失效分类相关的所有缓存"""
        invalidated_keys = set()
        try:
    ...
    def invalidate_template_related(
        self, template_id: int, game_gid: Optional[int] = None
    ) -> Set[str]:
        """失效模板相关的所有缓存"""
        invalidated_keys = set()
        try:
