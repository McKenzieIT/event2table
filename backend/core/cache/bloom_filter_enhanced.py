...
SECURITY CHANGES:
- Uses pybloom_live's native tofile/fromfile for serialization
- Metadata stored separately in JSON with validation
- Prevents code injection vulnerabilities from pickle
...
class EnhancedBloomFilter:
    ...
    def _validate_loaded_data(self, data: dict) -> bool:
        ...
        if not isinstance(data['size'], int) or data['size'] <= 0:
            logger.error(f"Invalid size in bloom filter metadata: {data['size']}")
            return False
        ...
        if 'rebuild_count' in data and not isinstance(data['rebuild_count'], int):
            logger.error(f"Invalid rebuild_count in bloom filter metadata")
            return False
