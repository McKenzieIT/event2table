...
class CreateJoinConfig(Mutation):
    ...
    @authenticated
    @require_permission("write")
    def mutate(self, info, game_gid, name, **kwargs):
        ...
        try:
            ...
            return CreateJoinConfig(ok=True, joinConfig=config, errors=[])

        except Exception as e:
            logger.error(f"Error creating join config: {e}")
            return CreateJoinConfig(ok=False, joinConfig=None, errors=[str(e)])
...
class UpdateJoinConfig(Mutation):
    ...
    def mutate(self, info, id, **kwargs):
        ...
        try:
            ...
            return UpdateJoinConfig(ok=True, joinConfig=config, errors=[])

        except Exception as e:
            logger.error(f"Error updating join config {id}: {e}")
            return UpdateJoinConfig(ok=False, joinConfig=None, errors=[str(e)])
    ...
    def mutate(self, info, id):
        ...
        try:
            ...
            return DeleteJoinConfig(
                ok=True, message="Join configuration deleted successfully", errors=[]
            )

        except Exception as e:
            logger.error(f"Error deleting join config {id}: {e}")
            return DeleteJoinConfig(ok=False, message="", errors=[str(e)])
