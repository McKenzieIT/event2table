...
def api_create_game() -> Tuple[Dict[str, Any], int]:
    ...
    try:
        execute_write(
            ...
            (gid_value, name, ods_db),
        )
        CacheInvalidator.invalidate_key("dashboard_statistics")  # Clear dashboard cache
...
def api_update_game(gid):
    ...
    try:
        ...
        CacheInvalidator.invalidate_key("games.list")  # Clear cache after update
        CacheInvalidator.invalidate_key("dashboard_statistics")  # Clear dashboard cache
...
def api_batch_delete_games():
    ...
    try:
        ...
        CacheInvalidator.invalidate_key("games.list")  # Clear cache after delete
        CacheInvalidator.invalidate_key("dashboard_statistics")  # Clear dashboard cache
