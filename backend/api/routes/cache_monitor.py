...
def list_cache_keys():
    ...
    if redis_client is None:
        return jsonify({"success": False, "error": "缓存不可用"})
    ...
    try:
        ...
        # 移除前缀以便显示 (解码bytes为str)
        keys = [
            (
                key.decode("utf-8").replace(key_prefix, "")
                if isinstance(key, bytes)
                else key.replace(key_prefix, "")
            )
            for key in all_keys
        ]
        ...
        return jsonify(
            {
                ...
                "data": {
                    "total_keys": len(keys),
                    "keys": sorted(keys_with_ttl, key=lambda x: x["key"]),
                },
            }
        )
