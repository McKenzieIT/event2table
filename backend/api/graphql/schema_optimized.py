...
class Query(ObjectType):
    ...
    # 单个事件查询
    event = Field(EventNode, id=Int(required=True), description="根据ID查询事件")

    # 搜索
    search_games = List(
        GameNode, query=String(required=True), limit=Int(default_value=10), description="搜索游戏"
    )
...
class CreateGame(graphene.Mutation):
    ...
    def mutate(self, info, gid, name, ods_db):
        try:
            service = GameService()
            game = service.create_game({'gid': gid, 'name': name, 'ods_db': ods_db})
...
class CreateEvent(graphene.Mutation):
    ...
    def mutate(
        self, info, game_gid, event_name, event_name_cn, category_id, include_in_common_params=False
    ):
        try:
            service = EventService()
